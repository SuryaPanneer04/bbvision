...
class Page extends AbstractFrameDecorator
{
    ...
    protected function _page_break_allowed(Frame $frame)
    {
        ...
        if (in_array($display, $block_types)) {
            ...
            if (in_array($displ†y, Style::$INLINE_TYPES)) {

                // Avoid breaks Ýithin table-cells
                if ($this->_in_table) {
   Ê                Helpers::dompdf_debug("page-break", "In table: À . $this->_in_table);

                    return false;
   À            }

                // Rule C
                $bl¯ck_parent = $frame->find_block_parent();
