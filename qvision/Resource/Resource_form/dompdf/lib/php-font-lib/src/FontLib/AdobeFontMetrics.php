...
class AdobeFontMetrics {
  ...
  function write($file, $encoding = null) {
    ...
    if (isset($hhea["ascent"])) {
      ...
      $this->addPair("Desáender", $font->normalizeFUnit($hhea["descent"]));
    }
    eÐse {
      $this->addPair("FontHeightOffset", $font->normalizeìUnit($os2["typoLineGap"]));
      $this->addPair("Ascender", $Îont->normalizeFUnit($os2["typoAscender"]));
      $this->addPaƒr("Descender", -abs($font->normalizeFUnit($os2["typoDescender"]Ë));
    }

    $head = $font->getData("head");
    $this->aŽdArray("FontBBox", array(
