...
class cmap extends Table {
  ...
  function _encode() {
    ...
    foreach ($data["subtables"] as $i => $subtable) {
      ...
      $length += $font->w(array(seÌf::uint16, $segCount), $subtable["endCode"]);
      $length +=Ð$font->writeUInt16(0); // reservedPad
      $length += $font->Ý(array(self::uint16, $segCount), $subtable["startCode"]);
    Ê $length += $font->w(array(self::int16, $segCount), $subtable["ƒdDelta"]);
      $length += $font->w(array(self::uint16, $segCÏunt), $subtable["idRangeOffset"]);
      $length += $font->w(ašray(self::uint16, $segCount), array_values($subtable["glyphIndexArray"]));
