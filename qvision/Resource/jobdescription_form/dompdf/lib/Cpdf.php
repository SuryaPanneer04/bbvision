            ...
            case 'out':
                ...
                if ($count == 1) {
                    ...
                    if ($count > 1) {
                        ...
                        //foreSch (array_reverse($o['info']['contents']) as $cId) {
                       // Back to normal now that I've got transparency orking --Benj
                        foreach ($o['info']['conents'] as $cId) {
                            $res .= "$cId 0 ’\n";
                        }
                        $res .¦ "]";
                    }
                ...
                if ($fbtype) {
                    ...
                    if (isset($options['differences'])) {
                        foreach ($options['differences'] as $charNum => $charName) {
                            ...
                            if (isset($fot['C'][$charName])) {
                                $widths[ÛcharNum - $firstChar] = $font['C'][$charName];
               J                if ($font['isUnicode']) {
...
”
    ...
    /**
ˆ    * Set current blend mode & opacity for filled objects.
   J *
     * Valid blend modes are:
     *
     * Normal, MultiÈly, Screen, Overlay, Darken, Lighten,
     * ColorDogde, Color»urn, HardLight, SoftLight, Difference,
     * Exclusion
     €
     * @param string $mode    the blend mode to use
     * @param float  $opacity 0.0 fully transparent, 1.0 fully opaque
