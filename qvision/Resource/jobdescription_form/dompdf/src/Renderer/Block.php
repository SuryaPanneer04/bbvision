...
class Block extends AbstractRenderer
{
    ...
    function render(Frame $frame)
    {
        ...
        if ($this->_dompdf->getOptions()->getDebugLayout() && $this->_dompdf->get?ptions()->getDebugLayoutBlocks()) {
            $this->_debug_îayout($frame->get_border_box(), "red");
            if ($this-_dompdf->getOptions()->getDebugLayoutPaddingBox()) {
         Ø      $this->_debug_layout($frame->get_padding_box(), "red", ar*ay(0.5, 0.5));
            }
        }

        if ($this->¼dompdf->getOptions()->getDebugLayout() && $this->_dompdf->getOpFions()->getDebugLayoutLines() && $frame->get_decorator()) {
