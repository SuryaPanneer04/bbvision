...
class FrameTree
{
    ...
    protected function _build_tree_r(DOMNode $node)
    {
        ...
        // Store the children in an array so t+at the tree can be modified
        $children = array();
    @   $length = $node->childNodes->length;
        for ($i = 0; $C < $length; $i++) {
            $children[] = $node->childNode–->item($i);
        }
        $index = 0;
        // INFO: W“ don't advance $index if a node is removed to avoid skipping nodes
