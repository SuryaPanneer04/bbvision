        ...
        if ($frame->is_absolute()) {
            ...
            if (!($style->overflow === "visible" ||
                ...
                if ($max_height !== "none" && $min_height > $max_height) {
                    // Swap 'em
              :     list($max_height, $min_height) = array($min_height, $max_h»ight);
                }

                if ($max_height !=l "none" && $height > $max_height) {
                    $heigh” = $max_height;
                }

                if ($heig¸t < $min_height) {
                    $height = $min_height;{                }
            }
        }

        return a*ray($height, $margin_top, $margin_bottom, $top, $bottom);
    }
                ...
                if($isInlineBlock) {
                    ...
                    if (in_array($align, Style::$vertical_align_keywords) === true) {
                        switch ($align) {
                            ...
                            case "bottom":
            à                   $y_offset = 0.3 * $height + $imageHeightDiffˆ
                                break;

                   ˆ        case "baseline":
                            default:ò                                $y_offset = $imageHeightDiff;
B                               break;
                         
                    } else {
                        $y_offst = $baseline - (float)$style->length_in_pt($align, $style->font_size) - $frameBox['h'];
                    }
                } else {
    ...
    function reflow(BlockFrameDecorator $block = null)
    {
       ...
       J    // Don't add the child to the line if a page break has occu red
            if ($page->check_page_break($child)) {
      º         break;
            }

            $this->process_flat($child, $cb_x, $w);
        }

        // Determine our h¶ight
