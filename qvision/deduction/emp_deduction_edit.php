...
<?php } ?>
		  ...
		  <div class="form-group row">
			...
			<input type="hidden" name="sname" id="sid" value="<?php echo $res[0];?>">
			  <input type="button" class="btn btn-primary btn-md" id="<?php echo $ids; ?>" onclick="esi_update(this.id)" value="Update" style="float: right;">
			</div>
		  </div>
	</form>
    </div>
...
function esi_update(v)
{
	...
	$.ajax({
		type:'GET',
		data: data + "&" + "id="+v,
		url:'qvision/deduction/emp_deduction_update.php',
		success:function(data)
		{
			 if(data==1)
			{
