...
require '../../connect.php';
$ids=$_REQUEST['ids'];
$edit_id=$con->query("select * from payroll_employer_deduction_master where id='$ids'");
$res = $edit_id->fetch();
?>
...
<i class="fa fa-table"></i> <font style="font-size:18px;"><b> DEDUCTION EDIT </b></font>
<a onclick="esi_back()" style="float: right;" data-toggle="modal" class="btn btn-danger">Back</a>
</div>
    <div class="card-body">
	 <form method="POST">
...
<?php } ?>
		  ...
		  <div class="form-group row">
			...
			<input type="hidden" name="sname" id="sid" value="<?php echo $res[0];?>">
			  <input type="button" class="btn btn-primary btn-md" id="<?php echo $ids; ?>" onclick="employer_deduction_update(this.id)" value="Update" style="float: right;">
			</div>
		  </div>
	</form>
    </div>
...
{ 
	//alert("nnn");
	 $.ajax({
    type:"POST",
    url:"qvision/deduction/employer_deduction.php",
...
function employer_deduction_update(v)
{
	...
	$.ajax({
		type:'GET',
		data: data + "&" + "id="+v,
		url:'qvision/deduction/employer_deduction_update.php',
		success:function(data)
		{
			 if(data==1)
			{
