...
<table class="table"  id="dt-basic-checkbox" cellspacing="0" width="100%">
          ...
          <tr>
            ...
            <tbody>
               <?php
if($userrole == 'R002' || $userrole == 'R001' || $userrole == 'R004'){
$sql=$con->query("SELECT a.id,a.client_org,a.client_name,b.feedback,b.feedback_date,b.date,a.status FROM `crm_calls` a left join `crm_calls_feedback` b on  (a.id=b.calls_id) where ((a.status='2' or a.status='3') and a.flag='1') group by a.id  order by a.id desc");
...
$cnt=$cnt+1;
 ...
 $sql=$con->query("SELECT a.id,a.client_org,a.client_name,b.feedback,b.feedback_date,b.date,a.status FROM `crm_calls` a left join `crm_calls_feedback` b on  (a.id=b.calls_id) where (a.created_by='8' or a.created_by='12' or a.created_by='211') and a.status not in (2,3) group by a.id  order by a.id desc");
