...
<?php } } ?>
	   <br>
            <br>
            <?php
			$sel1=$con->query("select id,status from crm_calls where id='$id'");
...
<?php } ?>
  ...
  <input type="hidden" name="rid" id="rid" value="<?php echo $id;?>">
        ...
        ?>
            <?php
 if ($fet1['status'] == 2 ) {
	 if($client_org == 'Individual Customer' && $client_type == 3){
		 if($user_id == 19){
		?>
 ...
 <?php
			 ...
			 $lead = $con->query("select * from crm_calls a join new_client_master b on a.client_org=b.org_name where a.client_org='$client_org' and a.client_type!='3'");
			//echo "select * from crm_calls a join new_client_master b on a.client_org=b.org_name where a.client_org='$client_org' and a.client_type!='3'";
