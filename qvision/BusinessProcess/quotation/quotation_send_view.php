...
//echo $costsheet_id.'jjj';
$stmtd = $con->query("SELECT cost_sheet_no,enquiry_id from cost_sheet_entry where enquiry_id='$costsheet_id'");
//echo "SELECT cost_sheet_no,enquiry_id from cost_sheet_entry where enquiry_id='$costsheet_id'";
$rowd        = $stmtd->fetch();
if (!$rowd) {
    echo "<div style='color:red; padding:20px; text-align:center;'><h3>Quotation Data Not Found</h3><p>Cost sheet entry not found for this enquiry ID.</p></div>";
    return;
}
...
$rowf        = $stmtf->fetch();
if (!$rowf) {
    echo "<div style='color:red; padding:20px; text-align:center;'><h3>Quotation Data Not Found</h3><p>Could not retrieve max cost sheet.</p></div>";
    return;
}
...
$stmt = $con->query("SELECT a.id as costsheet_id,a.cost_sheet_no as cst_no,a.*,b.*,e.*,f.*,g.*,h.*,i.employee as acc_manager,i.mail,b.org_name as cname,p.*,ci.*,s.* from cost_sheet_entry a 
		 ...
		 left join enquiry i on (i.id=a.enquiry_id)
		where a.cost_sheet_no='$cost_sheet_nos' and a.status IN ('1','2') ");

$stmt->execute();
$row        = $stmt->fetch();

if (!$row) {
    echo "<div style='color:red; padding:20px; text-align:center;'><h3>Quotation Data Not Found</h3><p>Please ensure the Cost Sheet exists and has been generated correctly.</p></div>";
    return; // Stop further execution to prevent warnings
}
...
<div class="col-xs-4">
<input type="text" style="width: 553px !important;margin-left: 20px;" title="Please enter valid Mail id (use comma ,)" class="form-control" id="sendermail" name="sendermail" placeholder="Enter CC Mail ID">
</div>
</div>
...
<section class="content" id="content">
    ...
    <div class="container-fluid">
	   ...
	   <img src="../../ss-information/images/04.png" alt="ssinfo1rmation">$cc_id
	</div>-->
...
<!--<div id="regularform">--->
	  ...
	  <?php } */ ?>
                    ...
                    <table id="dataTable" width="350px" border="1" style="border-collapse:collapse;border: 1px solid !important;" class="table border m_b_0px">
                        ...
                        <tr>
                            <td colspan='4' style="padding:10px;"><b>Company E-Mail : </b>enquiry@quadsel.in</td>
                            <td colspan='4'><b>Company Phone No : </b>044 2250 2277</td>
							<td colspan='4'><b>GST NO : </b>33AAACQ0129P2ZF</td>
                        </tr>
                        <tr>
                        ...
                        <tr style="border:1px solid black; padding:10px;">
                            <td colspan='12' style="padding:10px;">
                                ...
                                <!-- b>Mobile No : </b>< ?php echo $row['mobile_no']; ?> --> <br />
                                <b>Dear Sir,</b><br />
				  ...
				  <th>TOTAL WITH GST</th>
                        ...
                        <?php
                        $query = $con->query("SELECT a.cost_sheet_no as cst_no,a.*,b.*,e.* from cost_sheet_entry a 
				 left join new_client_master b on(b.id=a.client_id) 
				 left join staff_master e ON e.candid_id=a.candid_id
				 where a.status IN ('1','2') and a.cost_sheet_no='$cost_sheet_no' order by a.id desc");
                        ...
                        while ($quote = $query->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            echo "<br/>";
                            $iteam_rate = ($quote['qty'] > 0) ? ($totalamt / $quote['qty']) : 0;
					...
					<INPUT type="button" class="btn btn-danger" value="Delete" onclick="deleteRow('dataTable')" />
                        ...
                        <tr>
                            <td colspan="10" align="right"><b>Net Amount</b></td>
                        ...
                        <tr style="border-bottom-style: hidden;">
                            <TD colspan="12" style="text-align:left;"> <b>Tax Summary</b> </TD>
                        </tr>
						<?php
                            ...
                            <TD colspan="12">
                                ...
                                <div style="margin-left: 610px;">
                                    <div class="row">
                                        ...
                                        <div class="col-md-5" style="padding-left: 65px;"> <?php echo $first_score; ?> </div>
                                    </div>
                                    <br />
                                    <div class="row">
                                        <div class="col-md-3">CGST </div>
                                        <!--<div class="col-md-2"> < ?php echo $cgst_per; ?> </div>--> 
                                        <div class="col-md-5" style="padding-left: 65px;"> <?php echo $second_score; ?> </div>
                                    </div>
                                    <br />
                                    <div class="row">
                                        <div class="col-md-3">IGST </div>
                                        <!--<div class="col-md-2"> < ?php echo $igst_per; ?> </div>-->
                                        <div class="col-md-5" style="padding-left: 65px;"> <?php echo $igst_score; ?> </div>
                                    </div>
                                    <br />
                                    ....................................................................................... <br />
                                    <div class="row">
                        ...
                        <div style="width: 100%;border-top: 1px solid black;border-bottom: 1px solid black;height:100px;">
                            ...
                            <div style="margin-left:50%;">
                                ...
                                <div style="width: 100%; padding: 10px;">
                                    <div style="width: 85%; height: 70px; float: left; padding-top: 15px;">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $("form[name='add_name']").on("submit", function(ev) {
            ...
            $.ajax({
                url: "qvisionnew/qvision/BusinessProcess/quotation/name.php",
                method: "POST",
                data: formData,
                cache: false,
                contentType: false,
