...
<?php 
 ...
 public $costsheet_id;
	...
	$stmt= $con->query("SELECT a.id as costsheet_id,a.cost_sheet_no as cst_no,a.*,b.*,e.*,f.*,g.*,h.*,i.employee as acc_manager,g.user_name as imail,b.org_name as cname,p.*,ci.*,s.* from cost_sheet_entry a 
		 inner join new_client_master b on(b.id=a.client_id) 
 ...
 <table width="710px" style="text-align:left; border: none;">
	<tr><td colspan="5"><img src="../../../images/Q.png" alt="quadsel" style="width:160px; height:50px;"><br/></br>
...
<TABLE width="710px" style="border: 1px solid black;">
 <tr style="border:1px solid black;"> 
	...
	<tr style="border :1px  solid  black;font-size:13px;"> 
		...
		<b>Quote.NO : </b>'. $QuoteNo.' <br/><br/>
		<b>Date : </b>'. $quote_date.' 
		</td>
		
		<td><b>Mobile No : </b>'. $row['mobile_no'].'<br/><br/>
		<b>Ref.No.  : </b>'. $row['quote_no'].' <br/><br/>
		<b>Currency : </b>'. $currency .'
		</td>
		
		<td><b>E-Mail : </b>'. $imail.' <br/><br/>
		<b>GST NO :</b> 33AAACQ0129P2ZF <br/><br/>
		<b>Company Phone No :</b> 044 22502277 
		</td>
	</tr> <br/> <br/>
	  
	  <tr style="border:1px solid black;border-top-style: hidden;font-size:13px;">
	  <td colspan="3" style="font-size:13px;"><br/> <b><u>Client Name & Details : </u></b><br/><br/>'. $row['org_name'].'</b><br/><b>Contact Person &nbsp; : &nbsp; </b> '. $row['contact_person'].'<br/>
	  <b> Address &nbsp; : &nbsp; </b>'. $row['address'].','. $row['area'].',<br/>'. $row['city_name'].','. $row['pincode'].',<br/>'. $row['statename'].' .</b><br/>
	  <br/>
	  <b>Dear Sir,</b><br/>
 ...
 <table id="dataTable" width="710px"  border="1" style="font-size:13px;border-collapse:collapse;border: 1px solid !important;" class="table border m_b_0px"> 
		 ...
		 $output .= '<TR>
		   ...
		   <TH colspan="1">'.$net_amt.'</TH>
		 </TR>

		 
		 <TR>
		   <TH colspan="11" style="text-align:right;">GRAND TOTAL </TH> 
		   <TH colspan="1">'.$grand_totals.'</TH>
		 </TR>
	   </table>
 ...
 //terms and condition query
	 ...
	 <div class="container2"><br/>
		 <div style="text-align:left;">
		   <img src="../../../images/Q.png" alt="Quadsel" style="width:160px; height:50px;">
		 </div><br/>
		  
		 <div style="text-align:center;font-weight:bold;page-break-inside: avoid;">QUOTATION</div>
<div style="margin-top: -300px;font-size:13px; min-width:708px; min-height:400px; border: 1px solid black;">
 <br/> 
...
<br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/>
     <div style="width: 100%;border-top: 1px solid black;border-bottom: 1px solid black;height:100px;">
		...
		<div style="height: auto;"> 
			...
			<div style="width: 50%;margin-top:10px;">
				<div style="width: 75%; height: 70px; float: left; background: white;"> 
				   <div style="margin-top:12px; padding-left: 5px;"> Net Amount   : </div>
				</div>
		...
		<div style="margin-left:65%;justify-content: center;">
		   ...
		   <div style="font-weight:bold;">'. $row2['desg'].'</div>
...
/* 	echo "SELECT a.id as costsheet_id,a.*,b.*,e.*,f.*,g.*,h.* from cost_sheet_entry a 
    ...
    $sub='Quotation from Quadsel Systems by '.$emp_name1;
	
	$file_name = 'Quadsel'.'.pdf';
...
$mail->From = 'quote@quadsel.in';		//Sets the From email address for the message
	...
	$mail->AddAddress( $sendmail, $client_name);		//Adds a "To" address
	$mail->AddCC($cc_mail);
	$recipients = explode(',', $sendermail);
	foreach($recipients as $emailx)
{
    $mail->AddCC($emailx);
}
	//$mail->AddBCC('laxmipriya@bluebase.in');
   ...
   // $mail->AddAttachment( $file_to_attach,'$exxtra_file');
	...
	$mail->Body = "Dear&nbsp;&nbsp;$ccontact_person ,  <br> <br>
		...
		&nbsp;&nbsp;	Please Find the Quote in Attached File to this Email.   <br> <br>
	...
	echo "update quote_generate set status ='1', modified_by ='$candidateid',modified_on =NOW() WHERE quote_no= '$QuoteNo'";   		
	$update_query1 = $con->query("update cost_sheet_entry set status ='3', modified_by ='$candidateid',modified_on =NOW() WHERE cost_sheet_no= '$cost_sheet_nos'");
   echo "update cost_sheet_entry set status ='3', modified_by ='$candidateid',modified_on =NOW() WHERE cost_sheet_no= '$cost_sheet_nos'";
		$insert_query2= $con->query("Update enquiry set status='6' where id='$enquiry_id'"); 
		echo "Update enquiry set status='6' where id='$enquiry_id'"; 
	unlink($file_name);
