	<?php 
require '../../../connect.php';
require '../../../user.php';
require '../../../PHPMailer/PHPMailerAutoload.php';
$candidateid=$_SESSION['candidateid'];
$qid=$_REQUEST['id'];
$enquiry_id=$_REQUEST['enquiry_id'];
$cost_sheet_no=$_REQUEST['cost_sheet_no'];
$sel=$con->query("update po_generate set finance_status='1',finance_approved_by='$candidateid',po_upload_status='2' where id='$qid'");
//echo "update po_generate set finance_status='1',finance_approved_by='$candidateid' where id='$qid'";

$update_query = $con->query("update cost_sheet_entry set status ='9' WHERE cost_sheet_no= '$cost_sheet_no'");  
$insert_query2= $con->query("Update enquiry set status='10' where id='$enquiry_id'");

if($sel)
{
	
	echo 1;
}
else
{
	echo 0;
}

?>