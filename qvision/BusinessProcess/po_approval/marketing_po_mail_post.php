...
$deatsils=$con->query("SELECT pg.so_number,pg.quote_no,pg.cost_sheet_no,pg.po_date,pg.po_upload_status,zum.full_name,zum.user_name,cse.enquiry_id,cse.specification,cm.client_name,pg.marketing_approved_by FROM `po_generate` pg left JOIN z_user_master zum ON pg.created_by = zum.candidate_id left JOIN client_master cm on pg.created_by = cm.created_by left join cost_sheet_entry cse on pg.created_by=cse.candid_id where cse.cost_sheet_no=pg.cost_sheet_no and pg.cost_sheet_no = '$costsheet_no'");
...
SELECT pg.so_number,pg.quote_no,pg.cost_sheet_no,pg.po_date,pg.po_upload_status,zum.full_name,zum.user_name,cse.enquiry_id,cse.specification,cm.client_name FROM `po_generate` pg JOIN z_user_master zum ON pg.created_by = zum.candidate_id JOIN client_master cm on pg.created_by = cm.created_by join cost_sheet_entry cse on pg.created_by=cse.candid_id where cse.cost_sheet_no=pg.cost_sheet_no and zum.user_id='$candidateid'"); */
	...
	$mar_approve=$data['marketing_approved_by'];
	$mailerID=$data['user_name'];
	echo $mailerID;
	$sel=$con->query("select * from z_user_master where candidate_id='$mar_approve'");
	echo "select * from z_user_master where candidate_id='$mar_approve'";
	$sfet=$sel->fetch();
...
$mail->AddAddress($mailerID, $client);		//Adds a "To" addresss
