...
<tr>
	...
	<?php
		...
		if($emp_id){
		    $staff_sql=$con->query("SELECT a.* FROM staff_master a  join bb_attendance b on a.id = b.emp_code where a.id = '$emp_id' and  a.status=1 and year(b.in_log_date)='$from_year' and month(b.in_log_date) = '$from_month' group by b.emp_code ");	
			
		}else{
			$staff_sql=$con->query("SELECT a.* FROM staff_master a  join bb_attendance b on a.id = b.emp_code where  a.status=1 and year(b.in_log_date)='$from_year' and month(b.in_log_date) = '$from_month' group by b.emp_code ");
       ...
       while($staff_sql_res = $staff_sql->fetch()){
			...
			//Days of working		
			$days_sql=$con->query("SELECT total_no_of_days,days_worked FROM payroll_salary_deduction where payroll_year = '$from_year' and payroll_month = '$from_month' and employee_code='$employee_id' and total_no_of_days is not null limit 0,1");
			...
			//Earnings
			$earning_sql=$con->query("SELECT earnings,amount FROM payroll_salary_earnings WHERE payroll_year = '$from_year' and payroll_month = '$from_month' and employee_code='$employee_id' order by id asc");
		...
		//Earned salary START
			$earned_sql=$con->query("SELECT earnings,amount FROM payroll_earned_salary WHERE payroll_year = '$from_year' and payroll_month = '$from_month' and employee_code='$employee_id' order by id asc");
		...
		//Earned salary END
			...
			//deductions		
			$earning_sql=$con->query("SELECT * FROM payroll_salary_deduction WHERE payroll_year = '$from_year' and payroll_month = '$from_month' and employee_code='$employee_id' order by id asc");
