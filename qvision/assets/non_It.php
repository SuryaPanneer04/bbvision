...
<div  class="card card-primary">
			...
			<a onclick="add_nonit()"  style="float: right;" data-toggle="modal" class="btn btn-dark"><i class="fa fa-plus"></i> Add</a>
              </div>
...
<th>Action</th>
      ...
      $assets_sql=$con->query("SELECT * FROM `non_it_assets` n INNER JOIN assets_master a ON n.asset_name = a.id");
	   $i=1;
...
<script>
   ...
   function vms_edit(v){
	...
	$.ajax({
	type:"POST",
	url:"/KerliERP/Recruitment/vms_view.php?id="+v,
...
<script>
   	function add_nonit()
    {
     ...
     $.ajax({
    type:"POST",
	url:"/KerliERP/AssetsQ/assets_nonit_add.php",
    success:function(data){
    $("#main_content").html(data);
    }
    }) 
