...
<div class="card-primary">
         <div class="card-header">
            <h1 class="card-title"><font size="5"> Staff Resignation List</font></h1>
          </div>
...
<table class="dataTables-example table table-striped table-bordered table-hover" id="example1">
      ...
      <tbody>
      <?php
      $emp_sql=$con->query("SELECT *,s.emp_name as name,r.candidate_id as cid,r.id as id,r.status as status,s.id as sid FROM `resignation_form_details` r join staff_master s on r.candidate_id=s.candid_id order by r.id DESC "); //where r.status<>1
      $i=1;
      ...
      <tr>
		...
		<span style="color:orange;text-align:center;"><b>Waiting</b></span>
		  ...
		  <?php if($emp_res['status'] == 2 or $emp_res['status'] == 3 )
		   {
			  ?>
		  <button class="btn btn-info btn-sm" data-id="<?php echo $emp_res['id']; ?>" onclick="hr_approve(<?php echo $emp_res['id']; ?>)"> <i class="fa fa-mail">Action</i></button> <?php } ?>
		 ...
		 $sta_cou=$stats->fetch();
		 $stats_counts=$sta_cou['count'];
		 
		 if($staff_count==$stats_counts and $emp_res['status'] == 4)
		 {
			 ?>
		<button class="btn btn-info btn-sm" data-id="<?php echo $emp_res['id']; ?>" onclick="releving_letter(<?php echo $emp_res['id']; ?>)"> Relieving Letter </button>
		  <button class="btn btn-secondary btn-sm" data-id="<?php echo $emp_res['id']; ?>" onclick="exp_letter(<?php echo $emp_res['id']; ?>)"> Experience Letter </button>
		  ...
		  <input type="hidden" name="relieve" id="relieve" value="<?php echo $emp_res['relieving_date']; ?>">
	...
	$.ajax({
	type:"POST",
	url:"qvision/Recruitment/staff_resignation/hr_staff_view.php?id="+v+"&cid="+cid,
	...
	$.ajax({
	type:"POST",
	url:"qvision/Recruitment/staff_resignation/hr_approve.php?id="+v+"&cid="+cid,
	success:function(data)
	{
		$("#table_view").html(data);
	}
	 ...
	 function releving_letter(v)
	  {
		  ...
		  var relieve=$('#relieve').val();
	$.ajax({
	type:"POST",
	url:"qvision/Recruitment/staff_resignation/relieving_letter.php?id="+v+"&staff="+staff+"&staff_code="+staf_code+"&staff_name="+staff_name+"&applied_date="+applied_date+"&notice="+notice+"&relieve="+relieve,
	success:function(data)
	{
		$("#table_view").html(data);
	}
...
function exp_letter(v)
	...
	$.ajax({
	type:"POST",
	url:"qvision/Recruitment/staff_resignation/experience.php?id="+v+"&staff="+staff+"&staff_code="+staf_code+"&staff_name="+staff_name+"&designation="+designation+"&dep="+dep+"&notice="+notice+"&relieve="+relieve,
	success:function(data)
	{
		$("#table_view").html(data);
	}
