...
<div class="card card-primary">
	...
	<form method="POST" action="qvision/Recruitment/staff_asset/asset_recollect_submit.php">
...
<tr>
	...
	<?php
	$isel = $con->query("select distinct m.id as id,name,a.Serial_no as Serial_no from assets_form_detail a join assets_master m on a.asset_name=m.id where a.asset='Internal Asset' and m.name in('$access') and a.id in(select asset_id from staff_asset_list where staff_id='$sid' and status=1) ");
...
<table class="dataTables-example table table-striped table-bordered table-hover" id="example1">
	...
	<tr>
		<td>
			<?php
			$isel = $con->query("select distinct m.id as id,name,a.Serial_no as Serial_no,a.id as aid from assets_form_detail a join assets_master m on a.asset_name=m.name where a.asset='Internal Asset' and m.name in('$access') and a.id in(select asset_id from staff_asset_list where staff_id='$sid' and status=2)");
			...
			?>
				<div style="width:100%;float:left;padding: 5px 0px;">
					<div style="width:15%;float:left;margin-left: 113px;">
						...
						<input type="checkbox" name="View[]>" id="View<?php echo $i . $s++; ?>" value="<?php echo $dfet['aid']; ?>" />&emsp;<?php echo $dfet['name']; ?>
					</div>
