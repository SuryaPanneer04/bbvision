...
<div class="card card-info">
	...
	<script type="text/javascript">
		...
		function plant_insert(event) {
			...
			var pan_value = document.getElementById("sec").value;
			...
			var location = document.getElementById("Locationz").value;
			...
			if ($orge_type_value[0] != '7') {
				if (gst_value == "") {
					...
					$.ajax({
						type: 'POST',
						data: data,
						url: 'qvision/calls/plant_submit.php',
						success: function(result) {
							//alert(result)
							if (result.trim() == 1) {
								...
								alert("Please Fill All Required Fields (Organization, Location, City, GST/PAN, Status)");
							} else {
								alert("Error: " + result);
							}
						}
