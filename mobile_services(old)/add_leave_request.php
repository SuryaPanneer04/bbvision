    ...
    try {
        if (
            ...
            if ($leave_avail >= $no_of_day) {
                ...
                $stmt = $con->prepare("INSERT INTO leave_apply_masters
                    (candid_id, reporting_person_id, emp_code, emp_name, leave_type, req_date, leave_date, from_date, to_date, no_of_days, leave_reason, sick_doc, status, created_by, created_on)
                    VALUES
                    (?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW())
                    ");
