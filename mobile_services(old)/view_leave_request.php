...
try {
    $user_id   = $_REQUEST['userId'] ??  $_GET['userId'] ??  null;
    ...
    $query = $con->prepare("SELECT
        ml.leave_name,
        lap.*
    FROM leave_apply_masters lap
    LEFT JOIN master_leave ml
        ON ml.id = lap.leave_type
    WHERE (lap.candid_id = ? OR lap.reporting_person_id = ?)
